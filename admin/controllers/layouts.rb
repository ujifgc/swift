Admin.controllers :layouts do
  ...
  post :create do
    ...
    if @object.save
      ...
      File.open "#{Swift::Application.root}/views/layouts/#{@object.id}.slim", 'w', 0644 do |file|
  ...
  get :edit, :with => :id do
    @object = Layout.get(params[:id])
    @code = begin
      File.open "#{Swift::Application.root}/views/layouts/#{@object.id}.slim", "r:bom|utf-8" do |file|
        file.read + "\n"
      end
    rescue
      ''
  ...
  delete :destroy, :with => :id do
    @object = Layout.get(params[:id])
    if @object.destroy
      #File.rm "#{Swift::Application.root}/views/layouts/#{@object.id}.slim"
