 ...
 *---------------------------------------------------------------------------- */
var markupSet = function(textarea) {
    ...
    this.buttons = [
        ...
        new Wysiwym.Button('Inline Code', Wysiwym.span,  {prefix:'`',  suffix:'`',  text:'code'}),
	'|',                                  
        new Wysiwym.Button('Link',        Wysiwym.span,  {prefix:'[',  suffix:'](http://example.com)', text:'link text'}),
        new Wysiwym.Button('Page',        Wysiwym.pick,  {prefix:'[page #{id} ',   suffix:']', text:'#{title}', type:'page'}),
        new Wysiwym.Button('Asset',       Wysiwym.pick,  {prefix:'[asset #{id} ',  suffix:']', text:'#{title}', type:'asset'}),
        new Wysiwym.Button('Block',       Wysiwym.pick,  {prefix:'[block #{id} ',  suffix:']', text:'#{title}', type:'block'}),
        new Wysiwym.Button('Image',       Wysiwym.pick,  {prefix:'[image #{id} ',  suffix:']', text:'#{title}', type:'image'}),
	'|',                                  
	...
	'|',                                  
        ...
        new Wysiwym.Button('Header',      Wysiwym.block, {prefix:'### ', wrap:false}),
        new Wysiwym.Button('Code Block',  Wysiwym.block, {prefix:'    ', wrap:true}),
    '|',
        new Wysiwym.Button('Code',        Wysiwym.pick,  {type:'dropdown_codes?dialog'}),
        new Wysiwym.Dropdown('Code',      null,          {type:'dropdown_codes?drop'}),
    ];
    ...
    this.help = [
        ...
        { label: 'Inline Code Block', syntax: '`inline code`' }
    ];
};
...
Wysiwym.Button = function(name, callback, data, cssclass) {
    ...
    this.create = function() {
        var button = $("<div></div>").addClass('btn').attr('title', translation[this.name]).tooltip();
...
/*----------------------------------------------------------------------------------------------
 * Wysiwym Dropdown control
 * Represents a dropdown menu in the Wysiwym editor.
 *--------------------------------------------------------------------------------------------- */
Wysiwym.Dropdown = function(name, callback, data, cssclass) {
    this.name = name;                  // Button Name
    this.callback = callback;          // Callback function for this button
    this.data = data ? data : {};      // Callback arguments
    this.cssclass = cssclass;          // CSS Class to apply to button
...
// Wrap the selected text with a prefix and suffix string.
Wysiwym.pick = function(event) {
    var markup = event.data.markup;    // (required) Markup Language
    var prefix = event.data.prefix;    // (required) Text wrap prefix
    var suffix = event.data.suffix;    // (required) Text wrap suffix
    var text = event.data.text;        // (required) Default wrap text (if nothing selected)
    var type = event.data.type;        // (required) Object type to pick
    var wysiwym = new Wysiwym.Textarea(markup.textarea);
    if (wysiwym.selection.isWrapped(prefix, suffix)) {
        wysiwym.selection.unwrap(prefix, suffix);
        wysiwym.update();
    } else {
        var pick_callback = function(data) {
            var newtext   = text   ? text.replace(/#\{title\}/g, data['title']) : data['title'];
            var newprefix = prefix ? prefix.replace(/#\{id\}/g, data['id'])     : data['prefix'];
            var newsuffix = suffix ? suffix                                     : data['suffix'];

            if (wysiwym.selection.length() == 0) {
                wysiwym.selection.append(newtext);
            }else{
                newprefix += ' ';
            }
            wysiwym.selection.wrap(newprefix, newsuffix);
            wysiwym.update();
        };
        var objectType = type.toLowerCase();
        var url = '/admin/dialogs/' + objectType + 's?pick';
        var dialog_id = 'pick_'+objectType;
        var dialog = $('#'+dialog_id);
        if (dialog.length == 0) {
            dialog = $('<div class="modal loading hide" id="'+dialog_id+'"></div>').appendTo('body');
        }else{
            dialog_id = false;
        }
        // load remote content
        var pick_close = function() {
            pick_callback($(this).data());
            dialog.modal('hide');
            return false;
        };
        var bind_picks = function() {
